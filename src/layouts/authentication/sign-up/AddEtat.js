...
function AddEtat() {
  const [EtatValue, setEtatValue] = useState("");

  const handleEtatChange = (event) => {
    setEtatValue(event.target.value);
  };

  const handleAjouterEtat = async () => {
    try {
        ...
        <ArgonBox pt={2} pb={3} px={3}>
          <ArgonBox component="form" role="form">
            <ArgonBox mb={2}>
              <ArgonInput
                type="text"
                placeholder="Nom de la Etat"
                value={EtatValue}
                onChange={handleEtatChange}
              />
            </ArgonBox>
            <ArgonBox mt={4} mb={1}>
              <ArgonButton variant="gradient" color="dark" fullWidth onClick={handleAjouterEtat}>
                Ajouter
              </ArgonButton>
            </ArgonBox>
          </ArgonBox>
