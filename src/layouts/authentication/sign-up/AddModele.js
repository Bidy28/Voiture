...
function AddModele() {
  ...
  const handleModeleChange = (event) => {
    setModeleValue(event.target.value);
  };

  const handleAjouterModele = async () => {
    try {
        ...
        <ArgonBox pt={2} pb={3} px={3}>
          <ArgonBox component="form" role="form">
            <ArgonBox mb={2}>
              <ArgonInput
                type="text"
                placeholder="Nom de la Modele"
                value={ModeleValue}
                onChange={handleModeleChange}
              />
            </ArgonBox>
            <ArgonBox mt={4} mb={1}>
              <ArgonButton variant="gradient" color="dark" fullWidth onClick={handleAjouterModele}>
                Ajouter
              </ArgonButton>
            </ArgonBox>
          </ArgonBox>
